...
class Population:
    ...
    def __init__(self, seeder, flags, num_classes, maxloss=1000.):
        ...
        # Network parameters
        self.var_glorot_uniform('w0', [self.lmbda, 256, 196])
        self.var_glorot_uniform('w1', [self.lmbda, 128, 256])
        self.var_glorot_uniform('w2', [self.lmbda, 64, 128])
        self.var_glorot_uniform('w3', [self.lmbda, num_classes, 64])
        self.var_zero('b0', [self.lmbda, 256])
        self.var_zero('b1', [self.lmbda, 128])
        self.var_zero('b2', [self.lmbda, 64])
        self.var_zero('b3', [self.lmbda, num_classes])
    ...
    def var_glorot_uniform(self, name, shape):
        assert len(shape) == 3
        limit = tf.sqrt(6 / (shape[1] + shape[2]))
...
def evaluate(off, features, labels, batch_size):
    ...
    x = tf.tile(tf.expand_dims(features, 0), [off.lmbda, 1, 1])
    y = tf.tile(tf.expand_dims(labels, 0), [off.lmbda, 1, 1])

    # Network inference (batched over population and data)
    # General pattern: [pop, n, m] x [pop, batch, m] -> [pop, batch, n]
    for w, b in [(off.w0, off.b0), (off.w1, off.b1), (off.w2, off.b2)]:
        x = tf.einsum('pnm,pbm->pbn', w, x)
        x = x + tf.expand_dims(b, 1)
        x = tf.nn.relu(x)
    x = tf.einsum('pnm,pbm->pbn', off.w3, x)
    x = x + tf.expand_dims(off.b3, 1)
...
def evolutionary_algorithm(features, labels, num_classes, flags):
    ...
    with tf.device('/device:GPU:0'):
        ...
        with tf.name_scope('validation'):
            ...
            w3 = pop.w3[best_index]
            b0 = pop.b0[best_index]
            b1 = pop.b1[best_index]
            b2 = pop.b2[best_index]
            b3 = pop.b3[best_index]
            # Validation
            x = tf.reshape(features, [flags.batch_size, -1])
            for w, b in [(w0, b0), (w1, b1), (w2, b2)]:
                x = tf.einsum('nm,bm->bn', w, x)
                x = x + b
                x = tf.nn.relu(x)
            x = tf.einsum('nm,bm->bn', w3, x)
            val_logits = x + b3
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        with tf.name_scope('data_pipeline'):
            ...
            features_op = tf.layers.max_pooling2d(features_op, 2, 2)
        ...
        for batch in range(FLAGS.iterations):
            ...
            if (batch + 1) % FLAGS.val_interval == 0:
                ...
                h_val_acc.append([batch, test_acc_np])
                logger.info("val_acc: {:.4f}".format(test_acc_np))
